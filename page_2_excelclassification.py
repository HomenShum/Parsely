...
########################################################################################
    ...
    with st.expander("Pricing"):
        ...
        def display_pricing_information():
            st.header("OpenAI Model Pricing Overview")

            st.markdown("""
            OpenAI charges for usage based on the number of input and output tokens processed by the models. A token is approximately 4 characters or 0.75 words in English text.
            ...
            - **Output Tokens:** $12.00 per 1,000,000 tokens

            ### **o1-preview**

            - **Input Tokens:** $15.00 per 1,000,000 tokens
            - **Output Tokens:** $60.00 per 1,000,000 tokens

            **Note:** For the **o1** models (`o1-mini` and `o1-preview`), output tokens include internal reasoning tokens generated by the model that are not visible in API responses.
            """)

            st.markdown("""
            ### **Estimating Your Usage and Costs**

            To estimate the cost of using these models in your application, follow these steps:

            1. **Calculate Average Tokens per Request:**

                - **Input Tokens per Request:** Estimate the average length of your prompts in tokens.
                - **Output Tokens per Request:** Estimate the average length of the model's responses in tokens.

            2. **Estimate Monthly Usage:**

                - **Total Input Tokens per Month:** `Input Tokens per Request × Number of Requests per Month`
                - **Total Output Tokens per Month:** `Output Tokens per Request × Number of Requests per Month`

            3. **Compute Monthly Costs:**

                - **Input Cost:** `(Total Input Tokens per Month / 1,000,000) × Input Token Price`
                - **Output Cost:** `(Total Output Tokens per Month / 1,000,000) × Output Token Price`
                - **Total Monthly Cost:** `Input Cost + Output Cost`

            **Example Calculation:**

            Suppose your application processes **10,000** requests per month, and for each request:

            - **Average Input Tokens:** 500 tokens
            - **Average Output Tokens:** 200 tokens

            **Using GPT-4o-mini:**

            1. **Calculate Total Tokens:**

                - **Total Input Tokens:** 500 tokens/request × 10,000 requests = 5,000,000 tokens
                - **Total Output Tokens:** 200 tokens/request × 10,000 requests = 2,000,000 tokens

            2. **Compute Costs:**

                - **Input Cost:** (5,000,000 / 1,000,000) × $0.15 = **$0.75**
                - **Output Cost:** (2,000,000 / 1,000,000) × $0.60 = **$1.20**
                - **Total Monthly Cost:** $0.75 + $1.20 = **$1.95**

            **Using o1-mini:**

            1. **Calculate Total Tokens:**

                - **Total Input Tokens:** 5,000,000 tokens
                - **Total Output Tokens:** 2,000,000 tokens

            2. **Compute Costs:**

                - **Input Cost:** (5,000,000 / 1,000,000) × $3.00 = **$15.00**
                - **Output Cost:** (2,000,000 / 1,000,000) × $12.00 = **$24.00**
                - **Total Monthly Cost:** $15.00 + $24.00 = **$39.00**
            ...
            1. **Optimize Prompts and Responses:**

                - **Be Concise:** Keep prompts as short as possible while still conveying all necessary information.
                - **Limit Max Tokens:** Use the `max_tokens` parameter to limit the length of the model's response.

            2. **Monitor Usage:**

                - **Usage Dashboard:** Regularly check the OpenAI usage dashboard to track your token consumption.
                - **Set Budgets:** Implement usage limits or alerts to prevent unexpected charges.

            **Note:** You can find more detailed and up-to-date pricing information on the [OpenAI Pricing](https://openai.com/pricing) page.
            """)

        def display_pricing_calculator():
            st.header("Estimate Your Monthly Costs")

            model_choice = st.selectbox(
                'Select the model for estimation',
                ['GPT-4o', 'GPT-4o-mini', 'o1-mini', 'o1-preview']
            )

            input_tokens_per_request = st.number_input('Average Input Tokens per Request', value=500)
            output_tokens_per_request = st.number_input('Average Output Tokens per Request', value=200)
            number_of_requests_per_month = st.number_input('Number of Requests per Month', value=10000)
            ...
            elif model_choice == 'o1-mini':
                input_token_price = 3.00
                output_token_price = 12.00
            elif model_choice == 'o1-preview':
                input_token_price = 15.00
                output_token_price = 60.00

            # Calculate costs
            total_input_tokens = input_tokens_per_request * number_of_requests_per_month
            total_output_tokens = output_tokens_per_request * number_of_requests_per_month
            ...
            st.markdown(f"""
            ### **Estimated Monthly Costs for {model_choice}**
    ...
    with st.expander("Docx to CSV/Parquet/Excel Page Extractor", expanded=False):
    ...
    # Update the 'Select Use Cases' section
